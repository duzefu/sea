...
{
    if (magic_index>=0 && magic_index<MAX_GAME_SKILL_BASES)
    {
	    // (1) Break magic parts & append broken parts to zone magic parts list.
	    //   服務器只會炸開攻擊的法術效果
	    ExecBreakMagic(magic_index, npc_no, MAGIC_BREAK_TYPE_TAG, dest_x, dest_y, -1);
	    AppendBreakMagicParts();
	    // (2) Notify clients for break magic.
	    ServerSendNotifyBreakMagicToAll(npc_no, dest_x, dest_y, magic_index);
                                  ...
                                  SLONG magic_orig_x, SLONG magic_orig_y, SLONG magic_dest_x, SLONG magic_dest_y )
{
    ...
    for(dest_npc = 0; dest_npc < MAX_CHARACTER_PER_ZONE; dest_npc ++)
    {
        if(dest_npc != src_npc)
        {
            LOCK_CHARACTER(dest_npc);
            npc_flag = IsCharacterAvailable(dest_npc);
...
SLONG   server_check_is_magic_broken( SLONG magic_index, SLONG src_npc, SLONG map_no, SLONG magic_x, SLONG magic_y )
{
    ...
    //mark by lijun 2003-04-23
    //magic_adjacent = GetSkillExplodeDistance(magic_index);
    //magic_relation = GetSkillRelation(magic_index);
    ...
    for(dest_npc = 0; dest_npc < MAX_CHARACTER_PER_ZONE; dest_npc ++)
    {
        ...
        if(npc_flag && dest_npc_hp > 0)
        {
            ...
            if(distance < magic_adjacent)
            {
                ...
                {
                    broken_flag = TRUE;
                    dest_npc  = MAX_CHARACTER_PER_ZONE + 100;  //break the for loop.
                }
            }
        }
    }
...
void	server_play_magic_voice(SLONG map_no, SLONG magic_index, SLONG kind, SLONG map_x, SLONG map_y)
{
    //TODO:
}
...
void	do_magic_part_line(SLONG p)
{
	...
	else
	{
		...
		broken_flag = server_check_is_magic_broken( magic_index, src_npc, map_no, next_x, next_y);
		
		if(broken_flag)
		{
			//play_magic_voice(map_no, magic_index, MAGIC_VOICE_BROKEN, next_x, next_y);
...
void	do_magic_part_trace(SLONG p)
{
	...
	case STEP_ARRIVE:
		...
		//broken_flag = server_check_is_magic_broken(magic_index, src_npc, map_no, dest_x, dest_y);
		SetZoneMagicPartCurrPosition(p, dest_x, dest_y, dest_z);
		PrepareZoneMagicPartForExplode(p);
		break;
...
void	do_magic_part_roll(SLONG p)
{
	...
	else
	{
		...
		broken_flag = server_process_magic_roll(magic_index, src_npc, map_no, next_x, next_y, orig_x, orig_y,
			dest_x, dest_y);
...
void	do_magic_part_shake(SLONG p)
{
	...
	if(delay_tick <= 0)
	{
		...
		src_npc = GetZoneMagicPartSourceNo(p);
		magic_index = GetZoneMagicPartSkillIndex(p);
		server_process_magic_shake( magic_index, src_npc, map_no, next_x, next_y);
	}
...
void	do_magic_part_explode(SLONG p)
{
	...
	if(MAGIC_KEY_FRAME_HEAD == frame_type)
	{
		...
		//play_magic_voice(map_no, magic_index, MAGIC_VOICE_BROKEN, x, y);
	}
	else if(MAGIC_KEY_FRAME_OVER == frame_type)
	{
		SetZoneMagicPartFlag(p, 0);
